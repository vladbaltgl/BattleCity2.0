...
std::shared_ptr <Renderer::Texture2D> ResourceManager::getTexture(const std::string& textureName) {
	...
	std::cerr << "Can't find the shader texture:  " << textureName << std::endl;
	return nullptr;
}
															  ...
															  const std::string& subTextureName)
{
	...
	if (!pTexture) {
		std::cerr << "Can't find the texture: " << " for the sprite: " << spriteName << std::endl;
	}
																	   ...
																	   const unsigned int subTextureHeight)
{
	...
	if (pTexture)
	{
		...
		for (const auto& currentSybTextureName : subTextures)
		{
			...
			pTexture->addSubTexture(std::move(currentSybTextureName), leftBottomUV, rightTopUV);
